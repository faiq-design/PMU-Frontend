...
@Component({
  selector: "app-edit-security-invoice",
  templateUrl: "./edit-security-invoice.component.html",
  styleUrls: ["./edit-security-invoice.component.scss"],
  providers: [DecimalPipe], // Provide DecimalPipe here
})
export class EditSecurityInvoiceComponent implements OnInit {
  ...
  getServiceDetails() {
    ...
    this.invoiceService
      .getServiceDetails(this.invoice.service.id, this.invoice.vendor.id, this.invoice.hospital.id || -1)
      .subscribe(
        (response) => {
          if (response) {
  ...
  submitInvoice(){
    ...
    this.createInvoice(this.createInvoiceData, this.createInvoiceStatus ,  this.createInvoicepartialPayStatus);
  }
