...
export class EditInvoiceComponent implements OnInit {
  ...
  totalAmountWithoutTaxManual: number;
  taxAmountManual: number;
  totalNetAmountManual: number;
  remarkTypes: any = [];
  ...
  addAutoService() {
    ...
    let taxAmount = 0;
    let totalNetAmountManual = 0;
    let totalAmountWithoutTaxManual = 0;
    let taxAmountManual = 0;
    ...
    this.serviceDetails.forEach((element) => {
      if (element.hospitalId == this.invoiceForm.get("hospitalId").value) {
        ...
        if (i > -1) {
          if (element.auto == "Y") {
            totalNetAmount = totalNetAmount + parseFloat(element.netAmount);
            totalAmountWithoutTax =
              totalAmountWithoutTax + parseFloat(element.amount);
            taxAmount = taxAmount + parseFloat(element.taxAmount);
          } else if (element.auto == "N") {
            totalNetAmountManual =
              totalNetAmountManual + parseFloat(element.netAmount);
            totalAmountWithoutTaxManual =
              totalAmountWithoutTaxManual + parseFloat(element.amount);
            taxAmountManual = taxAmountManual + parseFloat(element.taxAmount);
          }
    ...
    totalNetAmountManual = parseFloat(totalNetAmountManual.toFixed());
    this.totalNetAmountManual = totalNetAmountManual;
    totalAmountWithoutTaxManual = parseFloat(
      totalAmountWithoutTaxManual.toFixed()
    );
    this.totalAmountWithoutTaxManual = totalAmountWithoutTaxManual;
    taxAmountManual = parseFloat(taxAmountManual.toFixed());
    this.taxAmountManual = taxAmountManual;
  ...
  calculateTotalAmount() {
    ...
    detailsArray.controls.forEach((control: FormGroup) => {
      ...
      } else if (control.get("isAuto").value == "N") {
        totalNetAmountManual =
          totalNetAmountManual + parseFloat(control.get("netAmount").value);
        totalAmountWithoutTaxManual =
          totalAmountWithoutTaxManual + parseFloat(control.get("amount").value);
        taxAmountManual =
          taxAmountManual + parseFloat(control.get("taxAmount").value);
      }
    });
    ...
    this.invoiceForm.patchValue({
      invoiceAmount:
        0 +
        (totalAmountWithoutTax + totalAmountWithoutTaxManual) -
        (taxAmount + taxAmountManual) -
        this.invoiceForm.get("panelty").value +
        this.invoiceForm.get("paneltyTax").value,
      invoiceAmountWithoutTax:
        totalAmountWithoutTax + totalAmountWithoutTaxManual,
      invoiceTaxAmount: taxAmount + taxAmountManual,
    });
    //     const invoiceAmount =
    //   totalAmountWithoutTax -
    //   taxAmount -
      ...
      .subscribe(
        (response) => {
          ...
          if (response) {
            ...
            console.log("this.remarkTypes: ", this.remarkTypes);
...
`;
  ...
  formatNumber(num) {
    const formattedNumber = num ? this.decimalPipe.transform(num, "1.0-0") : 0;
