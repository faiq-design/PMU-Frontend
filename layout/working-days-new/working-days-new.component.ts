...
@Component({
  selector: "'app-working-days-new",
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
  templateUrl: "./working-days-new.component.html",
  styleUrls: ["./working-days-new.component.scss"],
  providers: [DatePipe],
})
export class WorkingDaysNewComponent implements OnInit {
  loadingData: boolean;
  ...
  private initializeEvents() {
    ...
    this.events = [
      ...
      // {
      //   id: 3,
      //   title: 'Non editable and deletable event anddd farhannn',
      //   color: colors.red,
      //   start: this.yesterday
      // }
    ];
  ...
  beforeMonthViewRender(renderEvent: CalendarMonthViewBeforeRenderEvent): void {
    renderEvent.body.forEach((day) => {
      ...
      if (day.inMonth) {
        ...
        day.cssClass = "bg-gray non-editable";
      }
    });
  }
  // changeStatus(day) {
  ...
  updateSelectedHospital() {
    if (this.selectedHospitalId) {
      this.selectedHospital = this.hospitalList.find(
        (hospital) => hospital.id == this.selectedHospitalId
      );
  ...
  getDeploymentData() {
    ...
    this.refresh.next();
    this.invoiceService
      .getDeploymentData(this.startDate, this.endDate, this.selectedHospitalId)
      .subscribe(
        (response) => {
          if (response) {
  ...
  openModal(field: any): void {
    // this.selectedField = { ...field };  // Make a copy to avoid direct mutation
    this.showModal = true;
  }
